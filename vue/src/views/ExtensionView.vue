<script setup>
  ...
  import { faDownload, faPuzzlePiece, faShield, faStore } from '@fortawesome/free-solid-svg-icons';
  import { faBilibili, faChrome, faEdge, faGithub } from '@fortawesome/free-brands-svg-icons';


  const github_repository_link = 'https://github.com/sakmist/Bilibili-Lottery-Local-Extension';
    ...
    <div class="d-flex flex-column flex-lg-row align-items-start align-items-lg-center gap-4">
      <div class="flex-grow-1">
        ...
        <p class="mb-0 text-muted mt-2">
          遇到问题可在
          <a class="text-decoration-underline" :href="github_repository_link" target="_blank"><font-awesome-icon
              :icon="faGithub" /> GitHub仓库地址</a>
          提Issue反馈或者 
          <a class="text-decoration-underline" :href="bilibili_author_link" target="_blank"><font-awesome-icon
              :icon="faBilibili" />B站联系@sakmist</a>。
        </p>
      </div>
      <div class="d-flex flex-column flex-md-row align-self-stretch gap-2 w-100">
        <div v-if="isEdgeBrowser" class="d-flex flex-column gap-2 w-100">
          <a class="btn btn-outline-dark btn-lg fw-semibold px-4 d-flex align-items-center justify-content-center"
            :href="edgeStoreLink" target="_blank" rel="noopener">
          ...
          <a class="btn btn-link btn-sm fw-semibold px-0 align-self-start" :href="downloadLink" target="_blank"
            rel="noopener">
            下载手动ZIP安装包
          </a>
        </div>
        <div v-else class="d-flex flex-column flex-md-row gap-3 w-100">
          <div class="d-flex flex-column flex-grow-1 gap-2">
            ...
            <div class="d-flex flex-column gap-1 align-items-start">
              <a class="btn btn-outline-dark btn-sm fw-semibold px-3" :href="edgeStoreLink" target="_blank"
                rel="noopener">
                <font-awesome-icon :icon="faEdge" class="me-2" /> Edge 插件商店 一键安装
              </a>
              <p class="small text-danger fw-semibold mb-0">
                推荐使用 Edge，虽然效果一样支持安装，但是 Edge 已经上架商店（仅 Edge 浏览器可以使用该商店），点击即可安装，无需繁琐操作。
              </p>
